...
const SavedNeedList: React.FC<propsType> = (props: propsType) =>  {
  const {isFetching, data, error, status} = useSavedNeeds();
  console.log(data);
  return (
      <div className="w-full mt-10 sm:mt-3 overflow-y-scroll flex flex-col">
          ...
          {data?.map(n => <NeedItem need={n.need} key={n.id} saved savedNeedId={n.id} />)}
      </div>
  )
}
