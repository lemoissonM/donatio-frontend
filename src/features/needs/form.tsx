...
const NeedForm: React.FC<NeedFormType> = (props) => {
    ...
    return (
        <div className={`fixed inset-0 overflow-y-auto  flex flex-col justify-center transition-all delay-700 ease-linear bg-opacity-50 bg-[#000]  ${!props.isShowing ? "scale-0 bg-primary-200 z-30" : "z-30"} `}>
            <form onSubmit={handleSubmit(onSubmit)} className={`md:w-[40%] sm:[95%] drop-shadow-lg bg-primary-200 rounded-[15px] h-fit mx-auto p-5 mt-50 sm:mb-10 transition-transform ease-out delay-700 ${!props.isShowing ? "-translate-y-[1000px]" : "translate-y-0"}`}>
            ...
            <div className="md:grid md:grid-cols-2 gap-3">
                    <button className="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 md:mx-5 tablet:ml-5 sm:ml-1 tablet:w-[40%] sm:w-[97%] border-secondary-900   border-[1px]  text-secondary-900 rounded-[15px] mt-[20px] h-[40px] hover:bg-secondary-900 hover:border-2 hover:text-white" 
                    onClick={(e)=>{
                        e.preventDefault();
                        resetForm();
                    }}>Cancel</button>
