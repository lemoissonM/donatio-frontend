    ...
    return (
        <div className={`px-5 sm:px-2 sm:mb-5 ${style}`}>
            ...
            <div className="bg-primary-300 px-2 mt-2 rounded-[15px]">
                <select type={type || 'text'} {...register(name, validator)} className='text-sm px-4 py-2 bg-transparent sm:p-3 w-full mt-1 ' placeholder={placeholder}>
                    {options.map(o => <option className="text-blue bg-primary-300" value={o.value}>{o.label}</option>)}
                </select>
            </div>
        </div>
    )
