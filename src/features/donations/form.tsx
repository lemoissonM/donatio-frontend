...
const DonationForm: React.FC<DonationFormType> = (props) => {
    ...
    const {data: groups, isFetching: loadingUserGroups} = useUserGroup();
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="w-full bg-primary-200 rounded-[15px] p-5 mt-10 sm:mb-10">
           <h1 className="mx-5 tablet:mx-2 font-semibold mb-5 text-lg">Make a donation</h1>
           <div className="md:grid md:grid-cols-2 gap-3">
            <Input register={register} name="totalDonated" label="Amount" placeholder="Your amount here" />
            <Select  register={register} name="donationType" label="Type" placeholder="Your type here" options={[
